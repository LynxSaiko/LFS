...
class LazyFrameworkGUI(QMainWindow):
    def __init__(self):
        ...
        self.browser_controls_widget = None # <-- PERUBAHAN 1: Tambahkan referensi ini
        #self.load_banner()
        self.init_ui()
    ...
    def create_left_sidebar(self):
        ...
        # Tab widget untuk info dan browser - DIUBAH POSISI
    ...
    # === BROWSER PANEL METHODS - NEW ===
    def open_browser_panel(self):
        """Initialise and open the browser panel, replacing the placeholder."""
        if self.browser:
        ...
        # 1. Create Browser Control Widgets
        self.browser_controls_widget = QWidget() # <-- PERUBAHAN 2: Simpan referensi ke member
        ...
        # 3. Add to the browser tab layout
        # Get the layout of the "Browser" tab content (from create_left_sidebar)
        browser_tab = self.info_browser_tabs.widget(1)
        browser_tab_layout = browser_tab.layout()
        
        # Insert controls and browser view
        # The existing layout has: [0] control_layout, [1] browser_placeholder
        # Since we removed the placeholder, we insert the new controls and browser.
        browser_tab_layout.insertWidget(1, self.browser_controls_widget) # <-- PERUBAHAN 2: Gunakan referensi member
        browser_tab_layout.insertWidget(2, self.browser)

        self.browser_tab = browser_tab # Save reference to the tab widget
    ...
    def close_browser_panel(self):
        """Close the browser panel safely without closing the main GUI (FIXED LOGIC)"""
        if not self.browser:
            self.append_output("[dim]Browser is already closed.[/]")
            return

        try:
            # 1. Hentikan semua loading dan putuskan koneksi sinyal
            self.browser.stop()
            try:
                # Disconnect dengan argumen slot untuk menghindari error jika sinyal sudah terputus
                self.browser.urlChanged.disconnect(self.update_url_bar)
                self.browser.loadStarted.disconnect(self.on_load_started)
                self.browser.loadFinished.disconnect(self.on_load_finished)
            except Exception:
                pass
            
            # 2. Ambil layout tab Browser
            browser_tab = self.info_browser_tabs.widget(1)
            browser_tab_layout = browser_tab.layout()
            
            # 3. Hapus widget utama dari layout dan panggil deleteLater()
            
            # Hapus browser view
            if self.browser:
                browser_tab_layout.removeWidget(self.browser)
                self.browser.deleteLater()
            
            # Hapus container kontrol browser (Gunakan referensi yang tersimpan)
            if self.browser_controls_widget:
                browser_tab_layout.removeWidget(self.browser_controls_widget)
                self.browser_controls_widget.deleteLater()
            
            # 4. Reset semua references
            self.browser = None
            self.url_bar = None
            self.back_browser_btn = None
            self.forward_browser_btn = None
            self.refresh_browser_btn = None
            self.browser_controls_widget = None # <-- PERUBAHAN 3: Reset referensi container
            ...
            # Tambahkan placeholder ke posisi yang benar (setelah browser control buttons)
            browser_tab_layout.insertWidget(1, self.browser_placeholder)
            
            # 6. Reset proxy
            from PyQt6.QtNetwork import QNetworkProxy
            QNetworkProxy.setApplicationProxy(QNetworkProxy.ProxyType.NoProxy)
            
            self.append_output("[bold red]❌ Browser Panel Closed[/]")
            
        except Exception as e:
            self.append_output(f"[red]Error closing browser: {e}[/]")
            # Force cleanup
            self.browser = None
            QNetworkProxy.setApplicationProxy(QNetworkProxy.ProxyType.NoProxy)

    # Fungsi get_all_child_widgets dihapus karena sudah tidak diperlukan
