...
class LazyFrameworkGUI(QMainWindow):
    ...
    def open_browser_panel(self):
        ...
        # We need to replace [1] (placeholder) with new_control_layout and browser.
        # Since we removed the placeholder, let's just add the new elements after the initial control layout.
        
        # Find the index of the placeholder in the layout to insert new widgets
        browser_tab_layout.insertWidget(1, browser_controls)
    ...
    def close_browser_panel(self):
        """Close the browser panel safely without closing the main GUI"""
        if not self.browser:
        ...
        try:
            ...
            # 2. Putuskan semua koneksi sinyal
            try:
            ...
            # 3. Hapus browser dari layout
            browser_tab = self.info_browser_tabs.widget(1)
            browser_tab_layout = browser_tab.layout()
            
            # Cari dan hapus browser widget dari layout
            for i in reversed(range(browser_tab_layout.count())):
                item = browser_tab_layout.itemAt(i)
                if item and item.widget():
                    widget = item.widget()
                    if widget == self.browser:
                        browser_tab_layout.removeWidget(widget)
                        widget.deleteLater()
                        break
                    elif hasattr(widget, 'layout') and widget.layout():
                        # Cari di sub-layout (untuk browser controls)
                        sub_layout = widget.layout()
                        for j in reversed(range(sub_layout.count())):
                            sub_item = sub_layout.itemAt(j)
                            if sub_item and sub_item.widget():
                                sub_widget = sub_item.widget()
                                if sub_widget in [self.back_browser_btn, self.forward_browser_btn, 
                                                self.refresh_browser_btn, self.url_bar]:
                                    sub_layout.removeWidget(sub_widget)
                                    sub_widget.deleteLater()
            
            # 4. Hapus browser controls container jika ada
            control_widgets_to_remove = []
            for i in reversed(range(browser_tab_layout.count())):
                item = browser_tab_layout.itemAt(i)
                if item and item.widget():
                    widget = item.widget()
                    if (hasattr(widget, 'layout') and widget.layout() and 
                        any(sub_widget in [self.back_browser_btn, self.forward_browser_btn, 
                                        self.refresh_browser_btn, self.url_bar] 
                        for sub_widget in self.get_all_child_widgets(widget))):
                        control_widgets_to_remove.append(widget)
            
            for widget in control_widgets_to_remove:
                browser_tab_layout.removeWidget(widget)
                widget.deleteLater()
            
            # 5. Reset semua references
            self.browser = None
            self.url_bar = None
    ...
    def get_all_child_widgets(self, widget):
        ...
        if hasattr(widget, 'children'):
            for child in widget.children():
                if isinstance(child, QWidget):
                    ...
                    children.extend(self.get_all_child_widgets(child))
        return children
